...
/** Replace with final mockup images */
const CREATOR_IMAGE = "/gg.png";
const BRAND_IMAGE = "/ff.png";
...
const FeaturePill = ({ item, index, direction }) => (
  <motion.div
    ...
    className="flex items-center w-fit gap-x-2.5 pl-3 pr-5 py-2.5 rounded-full border border-[#E9E6E7] bg-white text-[#22000C] font-sans font-medium text-sm hover:bg-[#F9F6F6] transition-colors group cursor-default"
  >
...
const OpportunityBlock = ({
  ...
  <div
    className={`grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-[1fr_570px] gap-10 lg:gap-15 items-center px-5 xl:px-0 ${reverse ? "[&>*:first-child]:lg:order-2 [&>*:last-child]:lg:order-1" : ""
      }`}
  >
    <motion.div
      initial="hidden"
    ...
    >
      ...
      <motion.h2
        ...
        className="font-cormorant text-[36px] md:text-[40px] font-medium text-[#0D0D12] leading-[1.2] mb-5"
      >
        {title}
      </motion.h2>
      ...
      <motion.p
        ...
        className="text-[#22000C]/80 font-sans font-light leading-relaxed mb-6"
      >
        {description}
      </motion.p>
      ...
      >
        <p className="text-[#741717] text-lg font-sans">Apply for Early Access</p>
        <button
          type="button"
          onClick={onCta}
          className="h-14 px-10 rounded-full bg-[#741717] text-white font-sans text-base tracking-wide shadow-lg hover:bg-[#5a1212] transition-all hover:scale-[1.02] active:scale-[0.98]"
        >
          {ctaLabel}
        </button>
      </motion.div>
...
export const HomeOpportunities = ({ onJoinCreator, onJoinBrand }) => {
  return (
    <section className="w-full py-16 md:py-24 bg-white overflow-hidden">
      <div className="max-w-[1200px] mx-auto flex flex-col gap-20 md:gap-28">
        <OpportunityBlock
          tag="For Creators"
          title="Turn Your Creativity Into Opportunities"
          description="Build your portfolio, track your campaigns, and collaborate with brands that value your content."
          features={creatorFeatures}
          ctaLabel="Join as a Creator"
          onCta={onJoinCreator}
          imageSrc={CREATOR_IMAGE}
          imageAlt="Creator profile preview"
        />

        <OpportunityBlock
          tag="For Brands"
          title="Discover Creators Who Deliver Results"
          description="Launch campaigns with trusted creators. Access insights, streamline communication, and earn authentic reviews."
          features={brandFeatures}
          ctaLabel="Join as a Brand"
          onCta={onJoinBrand}
          imageSrc={BRAND_IMAGE}
          imageAlt="Brand profile preview"
          reverse
        />
      </div>
    </section>
